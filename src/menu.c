...
void MainMenu()
{
  ...
  Window *window;
  int done = 0;
  temp = IMG_Load("images/splash3.png");
  if(temp == NULL)
  {
  ...
  do
  {
    ...
    DrawMouse();
    if(lvl==1)done = 1;
	else if(keys[SDLK_ESCAPE] == 1)exit(1);
	NextFrame();
  }
  while(!done);
  CloseMouse();
...
void UpdateMainMenu()
{
	if((Mouse.buttons & SDL_BUTTON(1))&&(Mouse.oldbuttons == 0)){
		if(MouseIn(40,140,100, 60)){
			lvl=1;
			printf("You have decided to start a new game");
		}
...
void LoadDashboard()
{
  ...
  Dashboard.dashbutton = LoadSprite("images/dashbutton.png",50,100);
  Dashboard.dasharrows = LoadSprite("images/arrows.png",50,50);
